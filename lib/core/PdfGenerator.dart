...
class PdfGenerator {
  ...
  static pw.Widget _buildTablaRepuestos(List<Repuesto> repuestos) {
    return pw.TableHelper.fromTextArray(
      headers: ['Código', 'Descripción', 'Ubicación', 'Stock\nActual', 'Stock\nMínimo', 'Estado'],
      ...
      columnWidths: {
        0: const pw.FlexColumnWidth(1.5), 1: const pw.FlexColumnWidth(3),
        2: const pw.FlexColumnWidth(1.5), 3: const pw.FlexColumnWidth(1),
        4: const pw.FlexColumnWidth(1),   5: const pw.FlexColumnWidth(1.2),
      },
      data: repuestos.map((r) => [
        r.codigo, r.descripcion, r.ubicacion ?? '—',
        r.stockActual.toString(), r.stockMinimo.toString(),
        r.stockBajo ? '⚠ Bajo' : '✓ OK',
      ]).toList(),
  ...
  static pw.Widget _buildTablaSalidas(List<SalidaRepuesto> salidas) {
    return pw.TableHelper.fromTextArray(
      headers: ['Código', 'Descripción', 'Cantidad', 'Fecha', 'Ticket'],
      headerStyle: pw.TextStyle(fontSize: 9, fontWeight: pw.FontWeight.bold, color: PdfColors.white),
      ...
      columnWidths: {
        0: const pw.FlexColumnWidth(1.5), 1: const pw.FlexColumnWidth(3),
        2: const pw.FlexColumnWidth(1),   3: const pw.FlexColumnWidth(1.2),
      ...
      cellDecoration: (index, data, rowIndex) =>
          pw.BoxDecoration(color: rowIndex % 2 == 0 ? PdfColors.white : PdfColors.red50),
    );
  }
  ...
  static pw.Widget _buildTablaIngresos(List<IngresoRepuesto> ingresos) {
    return pw.TableHelper.fromTextArray(
      headers: ['Código', 'Descripción', 'Cantidad', 'Fecha', 'Quien entrega'],
      headerStyle: pw.TextStyle(fontSize: 9, fontWeight: pw.FontWeight.bold, color: PdfColors.white),
      ...
      data: ingresos.map((ing) => [
        ...
        '+${ing.cantidad}', ing.fecha, ing.quienEntrega,
      ]).toList(),
      cellDecoration: (index, data, rowIndex) =>
          pw.BoxDecoration(color: rowIndex % 2 == 0 ? PdfColors.white : PdfColors.green50),
    );
  }
  ...
  static pw.Widget _buildTablaMaquinas(List<Maquina> maquinas) {
    return pw.TableHelper.fromTextArray(
      ...
      cellStyle: const pw.TextStyle(fontSize: 9),
      columnWidths: {
        0: const pw.FlexColumnWidth(3),  1: const pw.FlexColumnWidth(1.5),
        2: const pw.FlexColumnWidth(2),  3: const pw.FlexColumnWidth(1.5),
      },
      data: maquinas.map((m) => [
        m.nombre, m.codigo, m.sectorNombre ?? '—',
        m.estado == 'en_reparacion' ? 'En reparación'
